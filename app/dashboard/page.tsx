...
import { getTopItems, logout } from "@/lib/spotify"
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch top tracks
        const tracksData = await getTopItems("tracks", {
        ...
        // Fetch top artists
        const artistsData = await getTopItems("artists", {
          limit: Number.parseInt(limit),
          time_range: timeRange,
        })
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8 items-start md:items-center justify-between">
          <Tabs defaultValue="tracks" className="w-full" onValueChange={setActiveTab}>
            <TabsList className="grid w-full max-w-md grid-cols-2">
              <TabsTrigger value="tracks" className="data-[state=active]:bg-green-500 data-[state=active]:text-black">
                <Music2 className="h-4 w-4 mr-2" /> Top Tracks
              </TabsTrigger>
              <TabsTrigger value="artists" className="data-[state=active]:bg-green-500 data-[state=active]:text-black">
        ...
        <div className="mt-8">
          {loading ? (
            ...
            <TabsContent value="tracks" className={activeTab === "tracks" ? "block" : "hidden"}>
          ...
          {loading ? null : (
            <TabsContent value="artists" className={activeTab === "artists" ? "block" : "hidden"}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
